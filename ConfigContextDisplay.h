...
namespace coverflow {
    ...
    void AppendDisplayContextMenuOptions(HMENU* hMenu) {
      int maxactionshortcuts = 10 - 1; //default is at pos 0 and at its index + 1
      CoverConfigMap coverconfigs = configData->CoverConfigs;
        ...
        uAppendMenu(_childDisplay, MF_STRING | session_pos == default_pos ? MF_DISABLED : MF_ENABLED,
            engine::ID_DISPLAY_0,
            PFC_string_formatter() << configData->CoverConfigSel.c_str() << " (default)"
            << "\t Ctrl + 0");

        int ndx = 0;
        for (auto& [name, config] : configData->CoverConfigs) {
          uAppendMenu(_childDisplay, MF_STRING | ndx == session_pos ? MF_CHECKED | MF_DISABLED
                                   : MF_UNCHECKED,
              engine::ID_DISPLAY_0 + ndx + 1,
              PFC_string_formatter() << name.c_str()
    ...
    void OnDisplayContextCommand(HMENU* hMenu, const int cmd, engine::EngineWindow* ew) {
        int ndx = cmd - engine::ID_DISPLAY_0 - 1; //remove first element (default)
        int default_pos = configData->GetCCPosition();
