...
EngineWindow::EngineWindow(HWND parent, ui_element_instance_callback_ptr defaultUiCallback)
	...
	glfwSetScrollCallback(glfwWindow.get(), [](GLFWwindow * window, double xoffset, double yoffset) {
		reinterpret_cast<EngineWindow*>(glfwGetWindowUserPointer(window))->onScroll(xoffset, yoffset); });
	glfwSetWindowRefreshCallback(glfwWindow.get(), [](GLFWwindow * window) {
		reinterpret_cast<EngineWindow*>(glfwGetWindowUserPointer(window))->onDamage(); });
	glfwSetWindowSizeCallback(glfwWindow.get(), [](GLFWwindow * window, int width, int height) {
		reinterpret_cast<EngineWindow*>(glfwGetWindowUserPointer(window))->onWindowSize(width, height);});
