...
EngineWindow::EngineWindow(HWND parent, ui_element_instance_callback_ptr defaultUiCallback)
	...
	SetWindowSubclass(hWnd, [](HWND, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR, DWORD_PTR dwRefData) {
		return reinterpret_cast<EngineWindow*>(dwRefData)->messageHandler(uMsg, wParam, lParam);
	...
	glfwSetScrollCallback(glfwWindow.get(), [](GLFWwindow * window, double xoffset, double yoffset) {
		static_cast<EngineWindow*>(glfwGetWindowUserPointer(window))->onScroll(xoffset, yoffset); });
	...
	glfwSetWindowSizeCallback(glfwWindow.get(), [](GLFWwindow * window, int width, int height) {
		static_cast<EngineWindow*>(glfwGetWindowUserPointer(window))->onWindowSize(width, height);});
...
void EngineWindow::onContextMenu(const int x, const int y) {
	...
	const int cmd = TrackPopupMenu(hMenu, TPM_NONOTIFY | TPM_RETURNCMD | TPM_RIGHTBUTTON | TPM_NOANIMATION,
