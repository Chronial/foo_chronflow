 ...
 public:
  ...
  HWND create_or_transfer_window(HWND wnd_parent,
    ...
    if (!window) {
      ...
      SetWindowPos(window->hwnd, NULL, p_position.x, p_position.y, p_position.cx,
  ...
  HWND get_wnd() const { return window->hwnd; }
  const bool get_is_single_instance(void) const { return true; }
};
