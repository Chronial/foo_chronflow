...
BOOL uRebar_InsertItem(HWND wnd, int n, uREBARBANDINFO* rbbi, bool insert) {
  ...
  if (IsUnicode()) {
    ...
    rbw.wID = rbbi->wID;
#if (_WIN32_IE >= 0x0400)  // we should check size of structure instead so keeping
                           // compatibility is possible, but whatever
...
BOOL uTabCtrl_InsertItemText(HWND wnd, int idx, const char* text, bool insert) {
  ...
  return insert ? uTabCtrl_InsertItem(wnd, idx, &tabs)
                : uTabCtrl_SetItem(wnd, idx, &tabs);
}
...
void strncpy_addnull(char* dest, const char* src, int max) {
  int n;
  for (n = 0; n < max - 1 && src[n]; n++) dest[n] = src[n];
  dest[n] = 0;
}
...
void convert_logfont_utf8_to_os(const uLOGFONT& p_src, LOGFONT& p_dst) {
  ...
  tcsncpy_addnull(p_dst.lfFaceName, pfc::stringcvt::string_os_from_utf8(p_src.lfFaceName),
                  tabsize(p_dst.lfFaceName));
}
...
void convert_logfont_os_to_utf8(const LOGFONT& p_src, uLOGFONT& p_dst) {
  ...
  p_dst.lfQuality = p_src.lfQuality;
  p_dst.lfPitchAndFamily = p_src.lfPitchAndFamily;
  strncpy_addnull(p_dst.lfFaceName, pfc::stringcvt::string_utf8_from_os(p_src.lfFaceName),
                  tabsize(p_dst.lfFaceName));
}
