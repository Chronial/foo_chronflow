...
int uHeader_InsertItem(
    ...
    bool insert = true);  // set insert to false to set the item instead
BOOL uToolTip_AddTool(HWND wnd, uTOOLINFO* ti, bool update = false);
BOOL uRebar_InsertItem(
    HWND wnd, int n, uREBARBANDINFO* rbbi,
    bool insert = true);  // set insert to false to set the item instead

BOOL uTabCtrl_InsertItemText(
    HWND wnd, int idx, const char* text,
    bool insert =
        true);  // fixes '&' characters also, set insert to false to set the item instead

inline void GetRelativeRect(HWND wnd, HWND wnd_parent,
...
HWND uRecursiveChildWindowFromPoint(
    HWND parent, POINT pt_parent);  // pt_parent is in parent window coordinates!
#endif
